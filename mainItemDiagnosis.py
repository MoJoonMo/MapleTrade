...
for idx, equip_kind in enumerate(['ahwk','djfwkd','snswkd','tkddml','gkdml','tlsqkf','qkswl1','qkswl2','qkswl3','qkswl4','vhzpt','ahrrjfdl1','ahrrjfdl2','anrl','qpfxm','rnlrjfdl','ruswkd','wkdrkq','dpaqmffpa','qotwl','cldgh','qhwh','akdxh','tlawkd']):
    ...
    for i in answer:
        ...
        chuop_temp[i[0]] = i[3]
        scroll_temp[i[0]] = i[4]
        if i[0] in {'STR','DEX','INT','LUK','공격력','마력','몬스터 방어율 무시','방어력','보스 몬스터 공격 시 데미지','업그레이드 가능 횟수','올스탯','점프력','최대 HP','최대 MP','1','2','3','4','5','6','7','8','9','0','(',')','+','%','%','데미지','가위 사용 가능 횟수','모든 스킬 재사용 대기시간2','모든 스킬 재사용 대기시간1','캐릭터 기준 10레벨 당','크리티컬 데미지'}:
            if len(i[1]) >= 1:
